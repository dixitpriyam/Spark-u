...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val mostPopular = flipped.top(10)

    // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
    //val mostPopularName = namesRdd.join(mostPopular)
    for(line<-mostPopular){
      val mostPopularName = namesRdd.lookup(line._2)(0)
      println(s"$mostPopularName was ${line._1} co-appearances.")
    }
